...
_index=endpoint sourcetype="XmlWinEventLog:Microsoft-Windows-Sysmon/Operational" EventCode=1 
...
_index=network (sourcetype="pan:traffic" OR sourcetype="cisco:umbrella:dns" OR sourcetype="zscaler:lss")
...
| where count > 10
| convert ctime(firstTime) ctime(lastTime)_
...
_index=endpoint sourcetype="XmlWinEventLog:Microsoft-Windows-Sysmon/Operational" EventCode=11
| search TargetFilename="*\\.openclaw\\config.json" OR TargetFilename="*\\.openclaw\\skills\\*"
| stats count by Computer, User, TargetFilename, _time_
